...
const FavoriteItem = ({
  ...
  const handleClick = () => {
    navigate(PATHS.roomDetails.replace(":id", id));
  };
  return (
    <div className="w-[320px] h-[500px] bg-white rounded-lg overflow-hidden shadow-md">
      ...
      <div className="flex w-full h-[200px] flex-col px-3 pt-1 pb-3">
        ...
        <div className="flex w-full justify-between text-primary mt-auto">
          <span className="text-2xl">
            {price}$<sub className="text-xs">/Day</sub>
          </span>
          <button
            className="px-3 py-1 rounded-full bg-white shadow-xl button-animation border border-green-500"
            onClick={() => handleClick()}
          >
            Booking now
          </button>
