...
const RoomUpdate = () => {
    ...
    http
      ...
      .then((res) => {
        ...
        reset({
          ...
          maxQuantityPeople: roomDetail.maxQuantityPeople,
        });
        const utilityResponses = [];
        roomDetail.utilitiesDtos.forEach((item, index) => {
          utilityResponses.push({
            name: `nameUtility${index}`,
            value: `valueUtility${index}`,
            index: index,
          });
          setValue(`nameUtility${index}`, item.name);
          setValue(`valueUtility${index}`, item.value);
        });
        const listImageUpdate = [];
        roomDetail.imageDtos.map((image) =>
  ...
  return (
    <form onSubmit={handleSubmit(addNewRoom)}>
      <h1 className="text-2xl font-bold mb-5">Update Rooms</h1>
